...
contract ERC1155Proxy is IERC1155Proxy, ERC1155PresetMinterPauserUpgradeable {
	...
	modifier onlyController() {
		require(_msgSender() == controller, "ERC1155Proxy: Sender must be the Controller");
	...
	modifier onlyOwner() {
		require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC1155Proxy: Caller is not the owner");
	...
	function mint(
		...
		uint256 amount,
		bytes memory data
	) public override(ERC1155PresetMinterPauserUpgradeable, IERC1155Proxy) onlyController {
	...
	function transferOwnership(address _newAdmin) public override onlyOwner {
		require(_newAdmin != _msgSender(), "ERC1155Proxy: cannot transfer ownership to existing owner");
	...
	function uri(uint256 id) public view override(ERC1155Upgradeable, IERC1155Proxy) returns (string memory) {
		return (_uris[id]);
	}
	...
	function tokenTotalSupply(uint256 id) external view override returns (uint256) {
	...
	function tokenTotalSupplyBatch(uint256[] memory ids) external view override returns (uint256[] memory) {

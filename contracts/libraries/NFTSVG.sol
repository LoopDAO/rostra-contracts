...
/// @title NFTSVG
/// @notice Provides a function for generating an SVG associated with a Paper NFT
library NFTSVG {
    ...
    return
      string(
        abi.encodePacked(
          ...
          generateSVGBorderText(params.creatorAddr, params.ownerAddr, params.creatorName, params.ownerName),
          generateSVGCardMantle(params.ownerName, params.creatorName, params.title),
          generageSvgCurve(10, 2000, 60, 9),
          generateSVGPositionDataAndLocationCurve(params.name, params.price, params.limit),
          generateSVGRareSparkle(params.tokenId, address(Address.parseAddr(params.ownerAddr))),
          '</svg>'
        )
      );
  }
  ...
  function generateSVGDefs(SVGParams memory params) private pure returns (string memory svg) {
    svg = string(
      abi.encodePacked(
        '<svg width="290" height="500" viewBox="0 0 290 500" xmlns="http://www.w3.org/2000/svg" xmlns:xlink=\'http://www.w3.org/1999/xlink\'><defs><filter id="f1"><feImage result="p0" xlink:href="data:image/svg+xml;base64,',
        Base64.encode(
          bytes(
            abi.encodePacked(
        ...
        '" /><feBlend mode="overlay" in="p0" in2="p1" /><feBlend mode="exclusion" in2="p2" /><feBlend mode="overlay" in2="p3" result="blendOut" /><feGaussianBlur in="blendOut" stdDeviation="42" /></filter> <clipPath id="corners"><rect width="290" height="500" rx="42" ry="42" /></clipPath><path id="text-path-a" d="M40 12 H250 A28 28 0 0 1 278 40 V460 A28 28 0 0 1 250 488 H40 A28 28 0 0 1 12 460 V40 A28 28 0 0 1 40 12 z" /><path id="minimap" d="M234 444C234 457.949 242.21 463 253 463" /><filter id="top-region-blur"><feGaussianBlur in="SourceGraphic" stdDeviation="24" /></filter><linearGradient id="grad-up" x1="1" x2="0" y1="1" y2="0"><stop offset="0.0" stop-color="white" stop-opacity="1" /><stop offset=".9" stop-color="white" stop-opacity="0" /></linearGradient><linearGradient id="grad-down" x1="0" x2="1" y1="0" y2="1"><stop offset="0.0" stop-color="white" stop-opacity="1" /><stop offset="0.9" stop-color="white" stop-opacity="0" /></linearGradient><mask id="fade-up" maskContentUnits="objectBoundingBox"><rect width="1" height="1" fill="url(#grad-up)" /></mask><mask id="none" maskContentUnits="objectBoundingBox"><rect width="1" height="1" fill="white" /></mask><linearGradient id="grad-symbol"><stop offset="0.7" stop-color="white" stop-opacity="1" /><stop offset=".95" stop-color="white" stop-opacity="0" /></linearGradient><mask id="fade-symbol" maskContentUnits="userSpaceOnUse"><rect width="290px" height="200px" fill="url(#grad-symbol)" /></mask></defs><g clip-path="url(#corners)"><rect fill="',
        params.color0,
        '" x="0px" y="0px" width="290px" height="500px" /><rect style="filter: url(#f1)" x="0px" y="0px" width="290px" height="500px" /> <g style="filter:url(#top-region-blur); transform:scale(1.5); transform-origin:center top;"><rect fill="none" x="0px" y="0px" width="290px" height="500px" /><ellipse cx="50%" cy="0px" rx="180px" ry="120px" fill="#000" opacity="0.85" /></g><rect x="0" y="0" width="290" height="500" rx="42" ry="42" fill="rgba(0,0,0,0)" stroke="rgba(255,255,255,0.2)" /></g>'
      )
    );
  }

  function generateSVGBorderText(
    string memory creatorAddr,
    string memory ownerAddr,
    string memory creatorName,
    string memory ownerName
  ) private pure returns (string memory svg) {
    string memory str1 = string(abi.encodePacked(ownerAddr, unicode' • ', ownerName));

    string memory str2 = string(abi.encodePacked(creatorAddr, unicode' • ', creatorName));
  ...
  function generateSVGPositionDataAndLocationCurve(
    ...
    (string memory xCoord, string memory yCoord) = rangeLocation(price, limit);

    string memory lenstr1 = string(
      abi.encodePacked(
        '<g style="transform:translate(29px, 384px)"><rect width="',
        (7 * ((bytes(name).length + 8) + 4) + 4).toString()
      )
    );

    string memory lenstr2 = string(
      abi.encodePacked(
        '</text></g><g style="transform:translate(29px, 414px)"><rect width="',
        (7 * ((bytes(priceString(price)).length + 10) + 4) + 4).toString()
      )
    );

    string memory lenstr3 = string(
      abi.encodePacked(
        '</text></g><g style="transform:translate(29px, 444px)"><rect width="',
        (7 * ((bytes(priceString(limit)).length + 10) + 4) + 4).toString()
      )
    );

    string memory s = string(
      abi.encodePacked(xCoord, 'px, ', yCoord, 'px, 0px)" cx="0px" cy="0px" r="4px" fill="white"/></g>')
    );
    svg = string(
      abi.encodePacked(
        lenstr1,
        'px" height="26px" rx="8px" ry="8px" fill="rgba(0,0,0,0.6)" /><text x="12px" y="17px" font-family="\'Courier New\', monospace" font-size="12px" fill="white"><tspan fill="rgba(255,255,255,0.6)">name: </tspan>',
        name,
        lenstr2,
        'px" height="26px" rx="8px" ry="8px" fill="rgba(0,0,0,0.6)" /><text x="12px" y="17px" font-family="\'Courier New\', monospace" font-size="12px" fill="white"><tspan fill="rgba(255,255,255,0.6)">Price: </tspan>',
        uint2str(price),
        lenstr3,
        'px" height="26px" rx="8px" ry="8px" fill="rgba(0,0,0,0.6)" /><text x="12px" y="17px" font-family="\'Courier New\', monospace" font-size="12px" fill="white"><tspan fill="rgba(255,255,255,0.6)">Limit: </tspan>',
        uint2str(limit),
        '</text></g><g style="transform:translate(226px, 433px)"><rect width="36px" height="36px" rx="8px" ry="8px" fill="none" stroke="rgba(255,255,255,0.2)" /><path stroke-linecap="round" d="M8 9C8.00004 22.9494 16.2099 28 27 28" fill="none" stroke="white" /><circle style="transform:translate3d(',
        s
      )
    );
  }

  function priceString(uint256 tick) private pure returns (string memory) {
    string memory sign = '';

    return string(abi.encodePacked(sign, uint256(int256(tick)).toString()));
  }

  function rangeLocation(uint256 tickLower, uint256 tickUpper) internal pure returns (string memory, string memory) {
    return ('24', '27');
  }
